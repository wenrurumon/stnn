...
write.csv(Y.fits,'summary2011/validation.csv')
...
write.csv(rlt.nat,'summary0211/prediction_national.csv')

#Futuring by city

out <- rbind(as.matrix(Y.actual)[1,],Y.model,t(out))
out <- apply(out,2,cumsum)
# out <- out[,match(colnames(out),names(sort(-colSums(out))))]
colnames(out) <- citymap$city[match(colnames(out),citymap$zh_name)]
out <- out[1:nrow(rlt.nat),]
for(i in 1:ncol(out)){plot.ts(out[,i],main=colnames(out)[i])}
write.csv(out,'summary0211/prediction_by_city.csv')
...
colnames(city.idx)[-1:-4] <- paste0('d',1:5)
plot(hclust(dist(scale(city.idx))),main='Feature Clustering')
write.csv(city.idx,'summary0211/city_idx.csv')
