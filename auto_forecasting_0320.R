...
raw2 <- read.csv('usa.csv')[,-1]; raw2 <- raw2[-nrow(raw2),]
raw3 <- read.csv("Cov19_316.csv") %>% 
...
names(raw) <- sapply(raw,function(x){toupper(x$state)})
raw[[which(names(raw)=='USA')]]$confirmed <- cumsum(rowSums(raw2[,-1]))
...
#                 ,(Y.raw[,1])[-1],sapply(model.vali,function(x){(x[,1])})),'/Users/wenrurumon/Documents/posdoc/wuhan/summary/temp.csv')
