...
temp <- data.frame(
  city=colnames(out),
  clust=kmeans(out[nrow(out),],9)$cluster,
  value=out[nrow(out),])
temp <- arrange(temp,desc(value));temp
temp2 <- out[,match(temp$city,colnames(out))]

par(mfrow=c(3,3))
for(i in unique(temp$clust)){
  k <- 0
  for(j in which((temp$clust)==i)){
    k <- k+1
    if(k==1){
      plot.ts((temp2[,j]),col=k,ylab='New Cases',xlab='Days')
    }else{
      lines((temp2[,j]),col=k,ylab='New Cases',xlab='Days')  
    }
    legend(1,1,
           lty=rep(1,k),legend=toupper(colnames(temp2)[which((temp$clust)==i)]),
           col=1:k,ncol=1,bg="white")
  }
}
temp2 <- rbind(t(temp),temp2)
write.csv(temp2,'summary0212/prediction_by_city4plot.csv')
...
city.kmeans <- lapply(1:10000,function(i){
  (city.kmeans <- kmeans((city.idx[,]),6,iter.max=10000)) 
  outer(city.kmeans$cluster,city.kmeans$cluster,'==')
})
for(i in 2:10000){city.kmeans[[1]] <- city.kmeans[[1]] + city.kmeans[[i]]}
city.kmeans.net <- ((1-city.kmeans[[1]]/10000)<0.1)
...
cbind(
  ...
  }))),'summary0212/clustering_result.csv')
...
ggplot() + geom_line(aes(x=rlt[-nrow(rlt),1],y=diff(rlt[,2]),colour='Actual')) +
  ...
  theme(plot.title = element_text(hjust = 0.5))
ggplot() + geom_line(aes(x=rlt.nat[1:90,1],y=(rlt.nat[1:90,2]),colour='Actual')) +
  geom_line(aes(x=rlt.nat[1:90,1],y=(rlt.nat[1:90,3]),colour='Prediction')) + 
