...
data.who2 <- sapply(who[7,2],read.who)
data.who2 <- as.data.table(t(data.who2[match(colnames(raw),rownames(data.who2)),]))
colnames(data.who2)[1] <- 'date'
data.who2[,1] <- as.numeric(paste0(who[7,1]))
data.who <- rbind(data.who,data.who2)
...
data.who2 <- sapply(who[8,2],read.who)
data.who2 <- as.data.table(t(data.who2[match(colnames(raw),rownames(data.who2)),]))
colnames(data.who2)[1] <- 'date'
data.who2[,1] <- as.numeric(paste0(who[8,1]))
data.who <- rbind(data.who,data.who2)
...
who <- read.xlsx('data0209.xlsx',sheet='who2')
read.who <- function(x,sel){
  x <- strsplit(x,'\r\n')[[1]]
  x <- strsplit(gsub('-',0,x),' ')
  x <- sapply(x,function(x){
    x.value <- which(!is.na(as.numeric(x)))[1]
    x.city <- paste(x[1:(x.value-1)],collapse='_')
    x.value <- x[(x.value):length(x)]
    c(x.city,x.value[sel])
  }) %>% t
  x <- data.table(city=tolower(x[,1]),
                  confirm=apply(x[,-1,drop=F],1,function(x){sum(as.numeric(x))}))
  x <- merge(x,citymap,by='city') %>% select(city=zh_name,confirm)
  rlt <- x$confirm
  names(rlt) <- x$city
  rlt
...
data.who2 <- sapply(1:ncol(who),function(i){
  read.who(who[1,i],sel=list(c(7,8),c(7,8),c(7,8),5,5)[[i]])
}) 
data.who2 <- as.data.table(t(data.who2[match(colnames(raw),rownames(data.who2)),]))
colnames(data.who2)[1] <- 'date'
data.who2[,1] <- as.numeric(colnames(who))
...
raw[[2]] <- round(raw[[2]] * (15384+36602)/36602)
raw <- rbind(raw,data.who2)
Y.actual <- raw[,-1]
...
dummy_data <- function(sel,p=8,h=1,w=NULL,x=Y.model,wl=6){
  # sel <- 3; p <- 8; h <- 1; w <- NULL; x <- Y.model; wl=6
...
write.csv(Y.fits,'summary_temp/validation.csv')
...
set.seed(3)
mfile <- dummy_data(sel=0,p=p,h=1,x=Y.model,wl=8)
...
for(i in 1:length(mmodel)){
  save_model_hdf5(mmodel[[i]]$model,paste0('mmodel',i,'.model')
                  , overwrite = TRUE,include_optimizer = TRUE)
  save_model_hdf5(mmodel[[i]]$encoder,paste0('mmodel',i,'.encoder')
                  , overwrite = TRUE,include_optimizer = TRUE)
}
setwd('..')
...
# mmodel[[1]]$model <- keras::load_model_hdf5('mmodel1.model')
# mmodel[[1]]$encoder <- keras::load_model_hdf5('mmodel1.encoder')
# mmodel[[2]]$model <- keras::load_model_hdf5('mmodel2.model')
# mmodel[[2]]$encoder <- keras::load_model_hdf5('mmodel2.encoder')
# mmodel[[3]]$model <- keras::load_model_hdf5('mmodel3.model')
# mmodel[[3]]$encoder <- keras::load_model_hdf5('mmodel3.encoder')
# mmodel[[4]]$model <- keras::load_model_hdf5('mmodel4.model')
# mmodel[[4]]$encoder <- keras::load_model_hdf5('mmodel4.encoder')
# mmodel[[5]]$model <- keras::load_model_hdf5('mmodel5.model')
# mmodel[[5]]$encoder <- keras::load_model_hdf5('mmodel5.encoder')
# setwd('..')
...
Y.fits <- sapply(mmodel,function(rlti){
  sapply(Xs,function(xi){
    x <- (rlti$model %>% predict(xi$x))*xi$f
    sum(ifelse(x<0,0,x))
  })
}) + rowSums(Y.actual[-1:-p,])

data.frame(
  date = (as.POSIXct('2020-01-18')+(1:nrow(Y.fits))*24*3600),
  actual = c(rowSums(Y.actual)[-1:-(p+1)],NA),
  fit = rowMeans(Y.fits))
...
rlt.nat <- data.frame(
  date = (as.POSIXct('2020-01-18')+(1:nrow(rlt.nat))*24*3600),
  rlt.nat
)
max(rlt.nat[,3],na.rm=T)
plot.ts(diff(rlt.nat[,3]))
...
data.frame(p=colnames(out),v1=apply(out,2,max),
           v2=colSums(Y.model),
           v3=apply(out,2,max)/colSums(Y.model)) %>% arrange(desc(v3))
write.csv(out,'summary_temp/prediction_by_city.csv')
...
city.idx <- apply(out,2,function(x){
  c(max = max(x),
    ...
    end1 = which((cumsum(x)/sum(x))>0.99)[1],
    end2 = max(which(x!=0)))
}) %>% t
...
colnames(city.idx)[-1:-6] <- paste0('d',1:5)
...
set.seed(1)
city.kmeans <- lapply(1:10000,function(i){
  # (city.kmeans <- kmeans((city.idx[,c(1:11)]),5,iter.max=10000))
  (city.kmeans <- kmeans(
    t(apply(city.idx[,c(1:11)],1,function(x){(x-min(x))/(max(x)-min(x))})),
    6,iter.max=10000))
  # (city.kmeans <- kmeans(
  #   t(apply(city.idx[,c(1:11)],1,function(x){(x/mean(x))})),
  #   4,iter.max=10000)) 
  outer(city.kmeans$cluster,city.kmeans$cluster,'==')
})
for(i in 2:10000){city.kmeans[[1]] <- city.kmeans[[1]] + city.kmeans[[i]]}
...
value <- merge(value,city.idx,by=c('city')) %>% mutate(city=toupper(city))
value$long <- as.numeric(paste(value$long))
value$lat <- as.numeric(paste(value$lat))
colnames(value)[c(6,16:17)] <- c('value','pattern','feature')
...
value$feature[14]<-'9'
value$pattern <- paste(match(paste(value$pattern),paste((value %>% group_by(pattern) %>% summarise(value=mean(as.numeric(paste(value)))) %>% arrange(desc(value)))$pattern)))
value$feature <- paste(match(paste(value$feature),paste((value %>% group_by(feature) %>% summarise(value=mean(as.numeric(paste(value)))) %>% arrange(desc(value)))$feature)))

ggplot(data=value, aes(x=long, y=lat, color=pattern)) + 
  geom_point(size=3) + 
...
ggplot(data=value, aes(x=long, y=lat, color=feature)) + 
  geom_point(size=3) + 
  geom_text(data=filter(value,pos=='a'),aes(x=long-nchar(city)/2.8,y=lat,label=city))+
  geom_text(data=filter(value,pos=='s'),aes(x=long,y=lat-1,label=city))+
  geom_text(data=filter(value,pos=='d'),aes(x=long+nchar(city)/3,y=lat,label=city))+
...
cbind(sapply(sort(unique(value$feature)),function(i){
  paste(filter(value,feature==i)$city,collapse=', ')
}))

write.csv(value,'summary_temp/city_idx.csv')

#Fit Chart

# rlt <- data.frame(date=1:22,apply(rlt.nat[1:23,],2,diff))
rlt <- data.frame(rlt.nat[1:24,])
rlt$date <- rlt$date + 24*3600
...
ggplot() + geom_line(aes(x=rlt[-nrow(rlt),1],y=diff(rlt[,2]),colour='Actual')) +
  ...
  theme(plot.title = element_text(hjust = 0.5))
ggplot() + geom_line(aes(x=rlt.nat[1:134,1],y=(rlt.nat[1:134,2]),colour='Actual')) +
  geom_line(aes(x=rlt.nat[1:134,1],y=(rlt.nat[1:134,3]),colour='Prediction')) + 
  labs(x='Day',y='Accumulated Confirmed Cases',title = "Prediction Chart") +
  theme(plot.title = element_text(hjust = 0.5))

#Prov level plot

out <- read.csv('summary_temp/prediction_by_city.csv')
out$X <- rlt.nat$date - 8*24*3600
colnames(out) <- toupper(colnames(out)) 
colnames(out)[c(1,8,14,25)] <- c('date','TAIWAN','HONGKONG','MACAO')
set.seed(3);out.clust <- kmeans(apply(out[,-1],2,max),9,iter.max=1000)$cluster
max(table(out.clust))
out.clust[] <- match(out.clust,as.numeric(names(sort(-tapply(apply(out[,-1],2,max),out.clust,mean)))))
out <- out[1:142,]
# out <- out[1:(max(which(round(diff(rowSums(out[,-1])))!=0))+1),]
# out <- data.frame(date=(out$date[-1]),apply(out[,-1],2,diff))
...
p <- ggplot() + geom_line(aes(x=temp[,1],y=temp[,2],colour='GUANGDONG')) + 
  ...
  geom_line(aes(x=temp[,1],y=temp[,3],colour=colnames(temp)[4]))+ 
  geom_line(aes(x=temp[,1],y=temp[,3],colour=colnames(temp)[5]))+ 
  labs(x='',y='',title = "") +
  theme(plot.title = element_text(hjust = 0.5))
print(p, vp = vplayout(1,2))
i <- 3
temp <- out[,c(1,which(out.clust==i)+1)]; ncol(temp); colnames(temp)[2]
p <- ggplot() + geom_line(aes(x=temp[,1],y=temp[,2],colour='ZHEJIANG')) + 
  geom_line(aes(x=temp[,1],y=temp[,3],colour=colnames(temp)[3]))+ 
  geom_line(aes(x=temp[,1],y=temp[,3],colour=colnames(temp)[4]))+ 
  labs(x='',y='',title = "") +
  theme(plot.title = element_text(hjust = 0.5))
...
i <- 4
temp <- out[,c(1,which(out.clust==i)+1)]; ncol(temp); colnames(temp)[2]
p <- ggplot() + geom_line(aes(x=temp[,1],y=temp[,2],colour='JIANGSU'))+  
  labs(x='',y='',title = "") +
  theme(plot.title = element_text(hjust = 0.5))
...
p <- ggplot() + geom_line(aes(x=temp[,1],y=temp[,2],colour='SHANGHAI'))+ 
  geom_line(aes(x=temp[,1],y=temp[,3],colour=colnames(temp)[3]))+ 
  geom_line(aes(x=temp[,1],y=temp[,4],colour=colnames(temp)[4]))+ 
  geom_line(aes(x=temp[,1],y=temp[,4],colour=colnames(temp)[5]))+ 
  labs(x='',y='',title = "") +
  theme(plot.title = element_text(hjust = 0.5))
...
p <- ggplot() + geom_line(aes(x=temp[,1],y=temp[,2],colour='GUANGXI')) + 
  ...
  geom_line(aes(x=temp[,1],y=temp[,4],colour=colnames(temp)[4]))+ 
  geom_line(aes(x=temp[,1],y=temp[,5],colour=colnames(temp)[5]))+ 
  geom_line(aes(x=temp[,1],y=temp[,5],colour=colnames(temp)[6]))+ 
  labs(x='',y='',title = "") +
  theme(plot.title = element_text(hjust = 0.5))
...
i <- 9
temp <- out[,c(1,which(out.clust==i)+1)]; ncol(temp); colnames(temp)[2]
p <- ggplot() + geom_line(aes(x=temp[,1],y=temp[,2],colour='MACAO')) + 
  geom_line(aes(x=temp[,1],y=temp[,3],colour=colnames(temp)[3]))+ 
  geom_line(aes(x=temp[,1],y=temp[,4],colour=colnames(temp)[4]))+ 
  geom_line(aes(x=temp[,1],y=temp[,5],colour=colnames(temp)[5]))+ 
  geom_line(aes(x=temp[,1],y=temp[,6],colour=colnames(temp)[6]))+ 
