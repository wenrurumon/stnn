...
# sel2 <- c('Italy','Spain','China','US',"UnitedKingdom") %>% toupper
# sel2 <- c('Newyork','ewjersey','california','michigan','massachusetts','louisiana') %>% toupper
...
#todate
todate <- function(x){
  x <- as.POSIXct( "2020-03-04") + 3600*24*x
  paste(x)
}
...
x1 <- fread("texas324.csv") %>% select(date,county,confirmed,metro_area)
x2 <- fread("texas411.csv") %>% select(date,county,confirmed=confirmed_new,metro_area) 
x <- rbind(x1,x2) %>% mutate(metro_area=ifelse(is.na(metro_area),'Others',metro_area)) %>%
  group_by(metro_area,date) %>% summarise(case=sum(confirmed))
raw.date <- paste(sort(as.POSIXct(unique(x$date))))

x <- sapply(unique(x$metro_area),function(i){
  x <- filter(x,metro_area==i)[match(as.POSIXct(raw.date),as.POSIXct(filter(x,metro_area==i)$date)),]$case
  ifelse(is.na(x),0,x)
})
...
for(i in 1:(365-nrow(Y.model))){
  ...
  x1 <- floor(ifelse(x1<0,0,x1)*sel)
  x2 <- floor(ifelse(x2<x1,x1,x2)*sel)
  x3 <- floor(ifelse(x3<x2,x2,x3)*sel)
  if(i %in% 1:7){
    temp <- rbind(temp,x2)
  } else {
    temp <- rbind(temp,x1)
...
rlts.index <- lapply(rlts,function(x){
  ...
  data.table(state=colnames(x),
    ...
    mutate(date_begin=todate(date_begin),date_max=todate(date_max),date_end=todate(date_end))
})
for(i in 2:5){
...
for(i in 1:length(rlts.index)){
  rlts.index[[i]] <- data.table(scenario=paste0('scenario',i),rlts.index[[i]])
}
write.csv(do.call(rbind,rlts.index),'/Users/wenrurumon/Documents/posdoc/wuhan/summary/temp.csv')

write.csv(do.call(cbind,lapply(rlts,function(x){x[,colnames(x)%in%c('total','Texas')]})),
