...
write.csv(test,'rlt_US_0425_10steperror_predict.csv')

x <- filter(raw,state=='US') %>% select(date,new)
x <- cbind(x,c(rep(NA,85),(do.call(rbind,lapply(rlt,function(x){filter(x,status=='prediction')[1,]})) %>% arrange(i))$new))
plot.ts(x[,2])
lines(x[,3],col=2)
colnames(x) <- c('date','Actual','Predicted')
x <- melt(x)
x$date <- as.Date(x$date)
ggplot(x,aes(colour=variable,y=value/1000,x=date)) + geom_line(size=1) + 
  labs(y='New Cases (K)',x = 'Date', title='New Cases - Total US | 10 step validation', colour='Legend') +
...
colnames(test) <- paste(1:ncol(test),'step error')
test <- (rbind(test,average=colMeans(abs(test),na.rm=T)))
write.csv(test,'rlt_US_0425_10steperror_fit.csv')
