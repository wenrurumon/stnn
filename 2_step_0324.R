...
X <- (t(sapply(mfile,function(x){
  x <- x$x
  c(x$new/mean(x$new+1),mean(x$d))
})))
Y <- cbind(sapply(mfile,function(x){as.numeric(x$y$d>0.5)}))
...
system.time(model %>% fit(x = X,y = Y,batch = 128,epochs = 1000,verbose = 0))
model %>% fit(x = X,y = Y,batch = 128,epochs = 100,verbose = 2)
keras::save_model_hdf5(model,paste0('/Users/wenrurumon/Documents/posdoc/wuhan/model/model2.model'),overwrite = TRUE,include_optimizer = TRUE)
