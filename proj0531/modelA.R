...
getpos <- function(i,train=T,h=8){
  x.pos <- t(pos[i+0:(h-1),])+1
  f.pos <- (rowMeans(x.pos))
  x.pos <- x.pos/f.pos
  x.trate <- t(trate[i+0:(h-1),])
  if(train){
    y.trate <- t(trate[i+h,,drop=F])
    y.pos <- (t(pos[i+h,,drop=F])+1)/f.pos
    y.a <- NULL
  } else {
    y.trate <- NULL
    y.pos <- NULL
...
getx <- function(i,train=T,h=8){
  x.pos <- t(pos[i+0:(h-1),])+1
  x.death <- t(death[i+0:(h-1),])+1
  f.pos <- (rowMeans(x.pos))
  f.death <- (rowMeans(x.death))
  x.pos <- x.pos/f.pos
  ...
  if(train){
    y.trate <- t(trate[i+h,,drop=F])
    y.pos <- (t(pos[i+h,,drop=F])+1)/f.pos
    y.death <- (t(death[i+h,,drop=F])+1)/f.death
    y.a <- NULL
  } else {
    y.trate <- NULL
    y.pos <- NULL
...
model <- function(X,Y){
  ...
  loss <- Inf
  itv <- 0
  system.time(temp <- model %>% fit(x = X,y = Y,batch = 128,epochs = 100,verbose = 0))n
...
ggplot() + geom_line(data=rlt,aes(x=date,y=value,colour=variable),size=1)
...
raw.weekday1 <- rep(raw.weekday[-1:-8],ceiling((1:length(data4train))/length(data4train)))
data4train <- rep(data4train,ceiling((1:length(data4train))/length(data4train)))

trate <- cbind(raw.trate)
pos <- cbind(raw.pos)
data4train2 <- lapply(1:(nrow(pos)-8),getpos,train=T) 
raw.weekday2 <- rep(raw.weekday[-1:-8],ceiling((1:length(data4train2))/40))
data4train2 <- rep(data4train2,ceiling((1:length(data4train2))/40))
data4train <- c(data4train,data4train2)
...
x.wd <- rep(c(raw.weekday1,raw.weekday2),sapply(data4train,function(x){x$x.pos%>%nrow}))
X <- cbind(x.pos,x.trate,weekday=outer(x.wd,1:7,'==')+0)
Y <- cbind(y.pos)

Y <- Y[rowSums(X[,1:8]-1==0)==0,]
X <- X[rowSums(X[,1:8]-1==0)==0,]
...
for(i in 1:length(model_positive)){
  save_model_hdf5(model_positive[[i]]$model,
                         paste0('/Users/wenrurumon/Documents/posdoc/wuhan/model/positivea_a_',i,'.model'))
}
# model_positive <- lapply(1:5,function(i){
#   x <- load_model_hdf5(paste0('/Users/wenrurumon/Documents/posdoc/wuhan/model/positive_a_',i,'.model'))
#   list(model=x)
# })
...
pos.pred <- lapply(model_positive,function(model){
  t(sapply(X,function(x){((model$model %>% predict(x$x))*x$f1-1) %>% floor}))
})
pos.pred <- sapply(pos.pred,rowSums)
pos.actual <- t(sapply(Y,function(x){x$y*x$f1-1})) %>% rowSums
...
pos.pred <- data.frame(date=as.Date(rownames(pos.pred)),
                       fit=(pos.pred),actual=pos.actual) %>% melt(id='date')
...
pos.pred <- lapply(model_positive,function(model){
  t(sapply(X,function(x){(model$model %>% predict(x$x))}))
})
for(i in 2:length(pos.pred)){pos.pred[[1]] <- pos.pred[[1]]+pos.pred[[i]]}
pos.pred <- pos.pred[[1]]/length(pos.pred)
pos.actual <- (t(sapply(Y,function(x){x$y})))
plot.ts(rowMeans((pos.pred+1)/(pos.actual+1)))

pos.pred <- sapply(pos.pred,rowSums)
pos.actual <- (t(sapply(Y,function(x){x$y*x$f1})) %>% rowSums)
plot.ts(((rowMeans(pos.pred))-pos.actual))
