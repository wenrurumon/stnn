...
log <- filter(raw,(scope%in%c('china')|state%in%c('KoreaSouth','Japan','Singapore','China'))
              &fact=='confirmed') %>% mutate(d=ifelse(d>0,1,0))
p <- 8
...
log <- filter(raw,scope%in%c('china','global')&fact=='confirmed') %>% mutate(
  d = ifelse(d>0,1,0)
)
p <- 8
mfile <- do.call(c,lapply(unique(log$key),function(K){
...
#1
temp <- filter(raw,scope=='global'&fact=='confirmed') %>% arrange(i)
for(j in 1:(365-(max(temp$i)))){
  # print(j)
  tempi <- do.call(rbind,lapply(unique(temp$key),function(K){
    logi <- getlog(max(temp$i)-7,K,temp)
    if(j %in% 1:7 & (logi$x$d[8]==0)){
      Di <- 0
    } else {
      Di <- 1
    }
...
plot.ts((temp %>% group_by(date) %>% summarise(new=sum(new)))$new)
...
write.csv(sapply(rlts,function(temp){
  (temp %>% filter(state=='Spain') %>% group_by(date) %>% summarise(new=sum(new)))$new 
}),'temp.csv')
