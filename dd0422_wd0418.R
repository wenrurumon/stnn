...
raw <- raw %>% mutate(d=ifelse(state=='Janpan',1,d))
raw <- raw %>% mutate(d=ifelse(state=='KoreaSouth',1,d))
raw <- raw %>% mutate(d=ifelse(state=='Singapore',1,d))
...
getdate <- function(x){
  paste(as.POSIXct('2020-01-22h')+3600*(x-1)*24)
}
...
keras::save_model_hdf5(model,paste0('/Users/wenrurumon/Documents/posdoc/wuhan/model/dd_d420.model'),overwrite = TRUE,include_optimizer = TRUE)
...
keras::save_model_hdf5(model,paste0('/Users/wenrurumon/Documents/posdoc/wuhan/model/dd_m0420.model'),overwrite = TRUE,include_optimizer = TRUE)

###################################
# Prediction
###################################

model <- keras::load_model_hdf5('/Users/wenrurumon/Documents/posdoc/wuhan/model/dd_m0420.model')
model.d <- keras::load_model_hdf5('/Users/wenrurumon/Documents/posdoc/wuhan/model/dd_d420.model')

# K <- 'global confirmed Singapore'
