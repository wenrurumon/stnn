...
sel2 <- c('DALLAS','HOUSTON')
ggstack2 <- function(temp,sel=0.75,sel2=sel2){
  colnames(temp) <- toupper(colnames(temp))
  temp <- data.table(date=as.POSIXct( "2020-03-03") + 3600*24*1:nrow(temp),temp) %>% as.data.frame
...
#todate
todate <- function(x){
  x <- as.POSIXct( "2020-03-03") + 3600*24*x
  paste(x)
}
...
x1 <- fread("texas324.csv") %>% select(date,state,county,confirmed,metro_area)
x2 <- fread("texas330.csv") %>% select(date,state,county,confirmed=confirmed_new,metro_area)
...
for(i in 1:ncol(temp)){
  for(j in nrow(Y.model):nrow(temp)){
    if(mean(temp[-1:-7+j,i])==0){temp[j,i]<-0}
  }
}
