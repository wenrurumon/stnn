...
raw4 <- read.csv("/Users/wenrurumon/Documents/posdoc/wuhan/data/github/global.csv")
raw.date <- raw4$X
raw.c <- raw4[,-1]
...
# test <- sapply(rlts,function(x){
#   x[,colnames(x)=='US']
# })
rlts <- lapply(rlts,function(x){x[,colnames(x)=='US',drop=F]})
